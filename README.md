...
- ⌨️ простой TUI под стрелки: `↑/↓`, `Enter`, `/`, `s`, `m`, `f`, `c`, `q`;
...
- `Enter` — обновить карточку выбранной раздачи.
- `/` — перейти в поиск, `Enter` применить, `Esc` вернуться к таблице.
- `s` — синхронизация; если сайт недоступен, TUI пишет ошибку в лог и остаётся живым.
